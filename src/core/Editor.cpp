...
void Application::editorGui() {
    ...
    // Determine inspector dimensions
    float activeInspectorWidth = showInspector ? inspectorWidth : 0.0f;
    float activeVSplitter = showInspector ? splitterThickness : 0.0f;

    if (showInspector) {
        if (inspectorWidth < 200.0f) inspectorWidth = 200.0f;
        if (inspectorWidth > totalWidth - 300.0f) inspectorWidth = totalWidth - 300.0f;
        activeInspectorWidth = inspectorWidth;
    }

    float mainAreaWidth = totalWidth - activeInspectorWidth - activeVSplitter;
    if (mainAreaWidth < 200.0f) mainAreaWidth = 200.0f;

    // Determine bottom panel height (Console Log and/or Table Visualizer)
    bool hasBottomPanel = showConsole || showTableVisualizer;
    bool isBottomCollapsed = ConsoleLog::Get().IsCollapsed();
    TableVisualizer::Get().SetCollapsed(isBottomCollapsed);

    float currentConsoleH = 0.0f;
    float viewportHeight = totalHeight;

    if (hasBottomPanel) {
        currentConsoleH = isBottomCollapsed ? 28.0f : consoleHeight;
        viewportHeight = totalHeight - currentConsoleH - (isBottomCollapsed ? 0.0f : splitterThickness);
    ...
    ImGui::BeginChild("MainAreaPanel", ImVec2(mainAreaWidth, totalHeight), false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
        ...
        ImGui::BeginChild("ViewportPanel", ImVec2(mainAreaWidth, viewportHeight), false);
            if (ImGui::BeginTabBar("MainViewportTabs", ImGuiTabBarFlags_Reorderable)) {
                ...
                if (sceneViewports.empty()) {
                    // Primary Default Scene Viewport
                        ...
                        if (availSize.x > 0 && availSize.y > 0) {
                            ...
                            if (is3DViewportHovered) {
                                ...
                                if (sizeP.x > 0.0f && sizeP.y > 0.0f) {
                                    ...
                                    viewportMouseNorm = Vector2{ normX, normY };

                                    if (ImGui::IsMouseReleased(ImGuiMouseButton_Left) && !ImGui::IsMouseDragging(ImGuiMouseButton_Left, 4.0f)) {
                                        On3DViewportClicked(viewportMouseNorm);
                                    }
                                }
                            }
                        } else {
                    ...
                    // Dynamic Scene Viewports (Multiple Workspaces)
                    int removeSceneIndex = -1;
                    bool anyHovered = false;

                    for (size_t i = 0; i < sceneViewports.size(); ++i) {
                        auto& svp = sceneViewports[i];
                        std::string tabLabel = svp.name + "###svp_" + std::to_string(svp.id);

                        if (ImGui::BeginTabItem(tabLabel.c_str(), svp.canClose ? &svp.open : nullptr)) {
                            activeSceneViewportIndex = (int)i;
                            ImVec2 availSize = ImGui::GetContentRegionAvail();

                            if (availSize.x > 0 && availSize.y > 0 && svp.isLoaded && svp.renderTexture.id > 0) {
                                    ...
                                    if (sizeP.x > 0.0f && sizeP.y > 0.0f) {
                                        ...
                                        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left) && !ImGui::IsMouseDragging(ImGuiMouseButton_Left, 4.0f)) {
                                            if (svp.clickCallback) {
                                                svp.clickCallback(svp, viewportMouseNorm);
                                            } else {
                                                On3DViewportClicked(viewportMouseNorm);
                                            }
                                        }
                        ...
                        if (!svp.open) {
                            removeSceneIndex = (int)i;
                        }
                    }

                    is3DViewportHovered = anyHovered;

                    if (removeSceneIndex >= 0) {
                        RemoveSceneViewport(removeSceneIndex);
                    }
                }
        ...
        if (hasBottomPanel) {
            // Horizontal Splitter Handle between Viewport and Console / Table Panel
            ...
            // Bottom Resizable Console / Table Panel
            float finalConsoleH = totalHeight - viewportHeight - (isBottomCollapsed ? 0.0f : splitterThickness);
            ImGui::BeginChild("ConsolePanel", ImVec2(mainAreaWidth, finalConsoleH), true);
                if (showTableVisualizer && showConsole) {
                    float halfW = (mainAreaWidth - 16.0f) * 0.5f;
                    if (halfW < 100.0f) halfW = 100.0f;

                    ImGui::BeginChild("TableWidgetChild", ImVec2(halfW, 0.0f), false);
                        TableVisualizer::Get().Draw("Table Visualizer");
                    ImGui::EndChild();

                    ImGui::SameLine();

                    ImGui::BeginChild("ConsoleWidgetChild", ImVec2(halfW, 0.0f), false);
                        ConsoleLog::Get().Draw("Console Log");
                    ImGui::EndChild();
                } else if (showTableVisualizer) {
                    TableVisualizer::Get().Draw("Table Visualizer");
                } else if (showConsole) {
                    ConsoleLog::Get().Draw("Console Log");
                }
    ...
    if (showInspector) {
        ...
        ImGui::BeginChild("InspectorPanel", ImVec2(inspectorWidth, totalHeight), true);
            ...
            if (!sceneViewports.empty() && activeSceneViewportIndex >= 0 && activeSceneViewportIndex < (int)sceneViewports.size()) {
                auto& activeSvp = sceneViewports[activeSceneViewportIndex];
                if (activeSvp.inspectorDrawCallback) {
                    activeSvp.inspectorDrawCallback(activeSvp);
                } else {
                    DrawInspectorUI();
                }
            } else {
